...
class Dnn:
    def __init__(self, dir_root='DNN DATA', model=None, transformer=None):
        # инициализация папок
    ...
    def create_regression(self, inputs=0, outputs=0, layers: list = (), activations: list = (), l_rate=0.01):
        return self._create(inputs, outputs, layers, activations, l_rate, 'mse', 'mae')

    def create_categorical(self, inputs=0, outputs=0, layers: list = (), activations: list = (), l_rate=0.01):
        return self._create(inputs, outputs, layers, activations, l_rate, 'categorical_crossentropy', 'accuracy')

    # компоновка и компиляция нейронной сети
    def _create(self, inputs=0, outputs=0, layers: list = (), activations: list = (), l_rate=0.01, loss='', metrics=''):
        assert inputs > 0
        assert outputs > 0
        assert 0.0 < l_rate < 1.0
        ...
        self.model.compile(loss=loss, optimizer=keras.optimizers.Adam(l_rate), metrics=[metrics])
        return self.model
    ...
    def fit(self, X_train_norm=None, Y_train=None, batch_size=1, epochs=1, dir_name=None):
        ...
        # сохранение модели
        self.model.save(os.path.join(self.dir_models, '{}.h5'.format(dir_name)))
        return self.model

    # оценка точнсти сети
    def evaluate(self, X_norm=None, Y=None):
        assert self.model is not None and X_norm is not None and Y is not None
        eval = self.model.evaluate(X_norm, Y, verbose=0)
        return eval
    ...
    def load_weights(self, path=None):
        try:
            ...
            lastest = train.latest_checkpoint(os.path.join(self.dir_weights, path))  # weights 04.18.2020 21.45
            assert path is not None, 'При загрузке весов произошла ошибка: path is None'
    ...
    def save_model(self, path=None):
        ...
        self.model.save(os.path.join(self.dir_models, path))

    def load_model(self, path=None):
        try:
            if path is None:
                path = self._get_last_saved_dir(self.dir_models)
            path = path.split('.')[0]
            assert path is not None, 'При загрузке весов произошла ошибка: path is None'
            self.model = keras.models.load_model(os.path.join(self.dir_models, '{}.h5'.format(path)))
        except Exception as e:

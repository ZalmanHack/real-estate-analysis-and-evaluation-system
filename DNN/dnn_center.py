...
class DnnCenter(Dnn):
    def __init__(self, dir_root='DNN DATA'):
        ...
        self.inputs = 2
        self.outputs = 1
        self.layers = [256, 256, 128, 128, 128, 128]
        self.activations = ['elu', 'elu', 'tanh', 'tanh', 'elu', 'elu']
        self.batch_size = 50
        # Нормализация
        self.x_tr = None
    ...
    def fit(self, X=None, Y=None, batch_size=40, epochs=100, dir_name='distance to center'):
    ...
    def load_weights(self, path='distance to center'):
        super().load_weights(path)

    def load_model(self, path='distance to center'):
        super().load_model(path)

    def save_model(self, path='distance to center'):

...
def model_fit(model=None, X_train_norm=None, Y_train=None, batch_size=1, epochs=1, device='cpu'):
    ...
    callback_lr = keras.callbacks.ReduceLROnPlateau(monitor='val_loss', factor=0.2, patience=5, min_lr=0.0005)
    tf.device('/{}:0'.format(device))
    # заполнитель (обучение)
    model.fit(X_train_norm, Y_train, batch_size=batch_size, epochs=epochs, verbose=10, validation_split=0.2,
...
def main():
    ...
    if t == '1':
        ...
        if t == '1':
            model = create_model(X_train_norm.shape[1])
            lastest = tf.train.latest_checkpoint('weights 04.18.2020 21.45')  # weights 04.17.2020 20.52  weights 04.18.2020 19.46
            print(lastest)
